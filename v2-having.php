...
<body>
    ...
    <form method="GET" action="v2-having.php"> <!--refresh page when submitted-->
    ...
    <form method="POST" action="v2-having.php"><!--refresh page when submitted-->

        <h2>Select Columns on Customer, Trainer, Train Table</h2> 
        <p style="color:grey;">Example: SELECT C.LastName, AVG(DISTINCT C.PrivateLessonRemaining) FROM Train T, Trainer S, Customer C WHERE S.StaffID=T.StaffID AND C.CustomerID=T.CustomerID GROUP BY C.LastName HAVING AVG(DISTINCT C.PrivateLessonRemaining) > 5;</FROM>
        ...
        <div  class="form-inline">
            ...
            <label for="lessonremaining">Private Lessons Remaining</label><br> </p>
                ...
                <label><input name="lm_aggregation" type="radio" value="AVG" checked>AVG</label>
    ...
    <h2>Select Columns To Group by</h2> 
        ...
        <h2>Select Having Conditions</h2> 
            <p>Please select the columns you want to groupby in the table Customer, your selection MUST be in selected columns above.</p>
            <div  class="form-inline">
                <p>HAVING: AVG(PrivateLessonsRemaining) </p>
                    <label><input name="operators" type="radio" value=">" checked>GT</label>
                    <label><input name="operators" type="radio" value="=">EQ</label>
                    <label><input name="operators" type="radio" value="<">LT</label>
                    <input type="number" id="conditionValue" name="conditionValue" value=5>
            </div>
    ...
    <hr />
        ...
        function handleGroupbyRequest() {
            global $db_conn;
            $result = executePlainSQL("SELECT C.LASTNAME, AVG(C.PRIVATELESSONREMAINING) FROM Train T, Trainer S, Customer C WHERE S.STAFFID=T.STAFFID AND C.CUSTOMERID=T.CUSTOMERID GROUP BY C.LASTNAME HAVING AVG(DISTINCT C.PRIVATELESSONREMAINING) > 5");
        ...
        function printGroupbyResult($result) { //prints results from a select statement
            ...
            echo "<table>";
            echo "<tr><th>LastName</th><th>Avg(PrivateLessonRemaining)</th></tr>";

            while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
                echo "<tr><td>" . $row["LASTNAME"] . "</td><td>" . $row["AVG(C.PRIVATELESSONREMAINING)"] . "</td></tr>"; //or just use "echo $row[0]"
            }
